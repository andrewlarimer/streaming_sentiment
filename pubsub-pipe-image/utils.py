...
def create_pubsub_client(credentials):
    ...
    return pubsub_v1.PublisherClient(
        batch_settings=BatchSettings(max_latency=5000, max_messages=50),
    )
    # http = httplib2.Http()
    # credentials.authorize(http)
    # return discovery.build('pubsub', 'v1beta2', http=http, batchsettings=BatchSettings(max_latency=5000, max_messages=50))

def create_old_pubsub_client(credentials):
    """Build the pubsub client."""
    # return pubsub_v1.PublisherClient(
    #     batch_settings=BatchSettings(max_latency=5000, max_messages=50),
    # )
